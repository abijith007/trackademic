...
def process_text_and_return_json(input_text):
    # Identify the title and description using the existing NLP code
    issue_type, title, description = identify_title_description(input_text)

    # Return the response in JSON format
    response_json = {"issue_type": issue_type, "title": title, "description": description}
    return json.dumps(response_json)

if __name__ == "__main__":
    # Check if the script is called with the correct number of command-line arguments
    if len(sys.argv) != 2:
    ...
    # Process the text and return the JSON response
    result_json = process_text_and_return_json(input_text)
